/**
 * Dashboard routes: statistics, MIS, audit trail, privacy review, the abuse
 * queue, readiness checks and stakeholder-network administration.
 *
...
export default function registerDashboardRoutes(router, deps) {
  const {
  ...
  function readinessChecks(actor) {
    ...
    const smsConfigured = Boolean(settings.smsGatewayUrl && settings.smsApiKey);
    return [
      {
        id: 'storage',
        label: 'Data storage',
      ...
      },
      {
        ...
        status: settings.exportSigning?.key ? 'pass' : 'warning',
        detail: settings.exportSigning?.key ? 'HMAC export signing key configured' : 'Demo HMAC export key in use; replace with managed key for production.',
      },
      {
        id: 'otp',
        label: 'OTP delivery',
  ...
  function signedExport(payload, type) {
    ...
    const keyId = settings.exportSigning?.keyId || 'demo-local';
    const key = settings.exportSigning?.key || 'khozo-demo-export-signing-key';
